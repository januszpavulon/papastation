...
/datum/mind/proc/show_memory(mob/recipient, window=1)
	if(!recipient)
		recipient = current
	var/output = "<B>Wspomnienia [current.real_name]:</B><br>"
	output += memory
	...
	if(objectives.len)
		...
		for(var/datum/objective/objective in objectives)
			output += "<br><B>Zadanie #[obj_count++]</B>: [objective.explanation_text]"
...
/datum/mind/Topic(href, href_list)
	...
	if (href_list["role_edit"])
		...
		switch(href_list["revolution"])
			if("clear")
				remove_rev()
				current << "<span class='userdanger'>Twoj mozg zostal wyprany! Juz nie jestes rewolucjonista!</span>"
			...
			if("rev")
				if(src in ticker.mode.head_revolutionaries)
					ticker.mode.head_revolutionaries -= src
					ticker.mode.update_rev_icons_removed(src)
					current << "<span class='userdanger'>Rewolucjonisci nie sa zbyt zadowoleni z twojego przywodztwa! Od teraz jestes zwyklym rewolucjonista!</span>"
				else if(!(src in ticker.mode.revolutionaries))
		...
		switch(href_list["traitor"])
			...
			if("traitor")
				if(!(src in ticker.mode.traitors))
					...
					current << "<span class='boldannounce'>Jestes Zdrajca!</span>"
...
/mob/living/carbon/human/mind_initialize()
	..()
	if(!mind.assigned_role)	mind.assigned_role = "Guwniak"	//defualt
