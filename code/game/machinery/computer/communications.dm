	...
	switch(href_list["operation"])
		...
		if("swipeidseclevel")
			...
			if (I && istype(I))
				if(access_captain in I.access)
					...
					usr << "<span class='warning'>Nie jestes do tego upowazniony!</span>"
					tmp_alertlevel = 0
				state = STATE_DEFAULT
			else
				usr << "<span class='warning'>Musisz zeskanowac swoj identyfikator!</span>"

		if("announce")
			if(src.authenticated==2 && !message_cooldown)
				make_announcement(usr)
			else if (src.authenticated==2 && message_cooldown)
				usr << "Systemy laduja sie. Prosze czekac."
		...
		if("nukerequest") //When there's no other way
			if(src.authenticated==2)
				...
				priority_announce("Otrzymano prosbe o otrzymanie kodow bezpieczenstwa wymaganych do uaktywnienia systemow autodestrukcji od [usr]. Informacja o pozytywnym lub negatywnym rozpatrzeniu prosby zostanie wkrotce podana..", "Kody Bezpieczenstwa",'sound/AI/commandreport.ogg')
				CM.lastTimeUsed = world.time
