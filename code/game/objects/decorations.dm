...
*/
	...
	attackby(obj/item/W as obj, mob/user as mob)
		...
		if ((user.mind && user.mind.assigned_role == "Kapitan") && istype(W, /obj/item/weapon/wirecutters))
			visible_message("<span class='notice'>[user] starannie rzezbi swoje drzewko.</span>")
			playsound(src, "sound/effects/ciemcie.huj.ogg", 100, 0)
		else if ((user.mind && user.mind.assigned_role == "Kapitan") && (!(istype(W, /obj/item/weapon/wirecutters))))
			visible_message("<span class='notice'>[user] niszczy drzewko bonsai.</span>")
		else if(istype(W, /obj/item/) && (user.mind && user.mind.assigned_role != "Kapitan"))
			src.update_icon()
			src.visible_message(text("<b><span style=\"color:red\">[] rujnuje [] przy pomocy [].</span></b>", user, src, W), 1)
			visible_message("<span class='notice'>Kapitan raczej nie bêdzie zadowolony.</span>")
