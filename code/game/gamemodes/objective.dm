...
/datum/objective/assassinate/update_explanation_text()
	..()
	if(target && target.current)
		explanation_text = "Zabij czlonka zalogi [target.name], jego praca to [!target_role_type ? target.assigned_role : target.special_role]."
	else
		explanation_text = "Nic"
...
/datum/objective/maroon/update_explanation_text()
	if(target && target.current)
		explanation_text = "Nie pozwol by [target.name], pracujacy jako [!target_role_type ? target.assigned_role : target.special_role], opuscil stacje zywy."
	else
		explanation_text = "Nic"
...
/datum/objective/debrain/update_explanation_text()
	..()
	if(target && target.current)
		explanation_text = "Ukradnij mozg czlonka zalogi [target.name], pracujacego jako [!target_role_type ? target.assigned_role : target.special_role]."
	else
		explanation_text = "Nic"
...
/datum/objective/protect/update_explanation_text()
	..()
	if(target && target.current)
		explanation_text = "Chron przed smiercia czlonka zalogi [target.name], pracujacego jako [!target_role_type ? target.assigned_role : target.special_role]."
	else
		explanation_text = "Nic"
...
/datum/objective/hijack
	explanation_text = "Porwij statek ewakuacyjny poprzez bycie jedyna zywa osoba na jego pokladzie po odleceniu od stacji."
	dangerrating = 25
...
/datum/objective/block
	explanation_text = "Nie pozwol by ktokolwiek uciekl ze stacji zywy."
	dangerrating = 25
	martyr_compatible = 1
...
/datum/objective/purge
	explanation_text = "Nie pozwol by jakikolwiek zmutowany osobnik uciekl ze stacji."
	dangerrating = 25
	martyr_compatible = 1
...
/datum/objective/robot_army
	explanation_text = "Miej przynajmniej 8 cyborgow polaczonych z toba."
	dangerrating = 25
	martyr_compatible = 0
...
/datum/objective/escape
	explanation_text = "Ucieknij ze stacji na pokladzie statku ewakuacyjnego zywy i wolny."
	dangerrating = 5
...
/datum/objective/escape/escape_with_identity/update_explanation_text()
	if(target && target.current)
		...
		explanation_text = "Ucieknij ze stacji na pokladzie statku ewakuacyjnego pod postacia czlonka zalogi [target_real_name], pracujacego jako [target.assigned_role]"
		...
		if(H && H.get_id_name() != target_real_name)
			...
			explanation_text += " posiadajac jego identyfikatorr"
...
/datum/objective/martyr
	explanation_text = "Zgin chwalebna smiercia."
	dangerrating = 1
...
/datum/objective/nuclear
	explanation_text = "Zniszcz stacje ladunkiem nuklearnym."
	martyr_compatible = 1
