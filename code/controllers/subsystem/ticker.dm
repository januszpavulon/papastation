...
/datum/subsystem/ticker/fire()
	switch(current_state)
		if(GAME_STATE_STARTUP)
			...
			world << "<b><font color='blue'>Witaj w lobby!</font></b>"
			world << "Jesli jeszcze tego nie zrobiles, ustaw swoja postac i przygotuj sie do gry. Runda zacznie sie za [config.lobby_countdown] sekund."
			...
			if(!mode.explosion_in_progress && mode.check_finished() || force_ending)
				...
				spawn(50)
					if(mode.station_was_nuked)
						world.Reboot("Stacja zostala zniszczona przez ladunek nuklearny.", "end_proper", "nuke")
					else
						world.Reboot("Runda zakonczona.", "end_proper", "proper completion")
...
/datum/subsystem/ticker/proc/setup()
	...
	if(!Debug2)
		if(!can_continue)
			qdel(mode)
			mode = null
			world << "<B>Wykryto blad podczas wlaczania trybu [master_mode].</B> Powrot do lobby."
			SSjob.ResetOccupations()
			return 0
	else
	...
	if(hide_mode)
		...
		world << "<B>Obecny tryb gry - Sekret!</B>"
		world << "<B>Mozliwosci:</B> [english_list(modes)]"
	else
		mode.announce()
	...
	world << "<FONT color='blue'><B>Witaj na [station_name()], zyczymy milego pobytu!</B></FONT>"
	world << sound('sound/AI/welcome.ogg')

	if(SSevent.holidays)
		world << "<font color='blue'>i...</font>"
...
/datum/subsystem/ticker/proc/equip_characters()
	...
	for(var/mob/living/carbon/human/player in player_list)
		if(player && player.mind && player.mind.assigned_role)
			if(player.mind.assigned_role == "Kapitan")
				captainless=0
	...
	if(captainless)
		for(var/mob/M in player_list)
			if(!istype(M,/mob/new_player))
				M << "Obecnie stacja nie posiada kapitana."
...
/datum/subsystem/ticker/proc/declare_completion()
	...
	world << "<BR><BR><BR><FONT size=3><B>Runda zakonczona.</B></FONT>"
			...
			if(Player.stat != DEAD && !isbrain(Player))
				...
				if(station_evacuated) //If the shuttle has already left the station
					if(!Player.onCentcom() && !Player.onSyndieBase())
						Player << "<font color='blue'><b>Udalo ci sie przezyc, jednak zostales porzucony na [station_name()]...</b></FONT>"
					else
						num_escapees++
						Player << "<font color='green'><b>Udalo ci sie przetrwac wydarzenia na [station_name()] jako [Player.real_name].</b></FONT>"
				else
					Player << "<font color='green'><b>Udalo ci sie przetrwac wydarzenia na [station_name()] jako [Player.real_name].</b></FONT>"
			else
				Player << "<font color='red'><b>Nie przetrwales wydarzen na [station_name()]...</b></FONT>"
	...
	world << "<BR>[TAB]Dlugosc zmiany: <B>[round(world.time / 36000)]:[add_zero("[world.time / 600 % 60]", 2)]:[world.time / 100 % 6][world.time / 100 % 10]</B>"
	world << "<BR>[TAB]Poziom integralnosci stacji: <B>[mode.station_was_nuked ? "<font color='red'>Zniszczona</font>" : "[station_integrity]%"]</B>"
	if(joined_player_list.len)
		world << "<BR>[TAB]Ilosc czlonkow zalogi: <B>[joined_player_list.len]</B>"
		if(station_evacuated)
			world << "<BR>[TAB]Ewakuowani: <B>[num_escapees] ([round((num_escapees/joined_player_list.len)*100, 0.1)]%)</B>"
		world << "<BR>[TAB]Zywi: <B>[num_survivors] ([round((num_survivors/joined_player_list.len)*100, 0.1)]%)</B>"
	world << "<BR>"

	//Silicon laws report
	for (var/mob/living/silicon/ai/aiPlayer in mob_list)
		if (aiPlayer.stat != 2 && aiPlayer.mind)
			world << "<b>[aiPlayer.name] (Grany przez: [aiPlayer.mind.key]) posiadal nastepujace prawa na koncu rundy:</b>"
			aiPlayer.show_laws(1)
		else if (aiPlayer.mind) //if the dead ai has a mind, use its key instead
			world << "<b>[aiPlayer.name] (Grany przez: [aiPlayer.mind.key]) posisdal nastepujace prawa na koncu rundy:</b>"
			aiPlayer.show_laws(1)

		world << "<b>Prawa zostaly zmienione [aiPlayer.law_change_counter] razy.</b>"

		if (aiPlayer.connected_robots.len)
			var/robolist = "<b>Podwladni [aiPlayer.real_name]:</b> "
			for(var/mob/living/silicon/robot/robo in aiPlayer.connected_robots)
				if(robo.mind)
					robolist += "[robo.name][robo.stat?" (Deaktywowany) (Grany przez: [robo.mind.key]), ":" (Grany przez: [robo.mind.key]), "]"
			world << "[robolist]"
	for (var/mob/living/silicon/robot/robo in mob_list)
		if (!robo.connected_ai && robo.mind)
			if (robo.stat != 2)
				world << "<b>[robo.name] (Grany przez: [robo.mind.key]) przetrwal jako cyborg nieposiadajacy kontrolujacego go AI. Jego prawa:</b>"
			else
				world << "<b>[robo.name] (Grany przez: [robo.mind.key]) nie przetrwal jako cyborg nieposiadajacy kontrolujacego go AI. Jego prawa:</b>"
...
/datum/subsystem/ticker/proc/send_random_tip()
	...
	if(randomtips.len)
		world << "<font color='purple'><b>Protip na dzis: </b>[html_encode(pick(randomtips))]</font>"
