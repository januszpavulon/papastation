...
/datum/subsystem/job/proc/GiveRandomJob(mob/new_player/player)
	...
	for(var/datum/job/job in shuffle(occupations))
		...
		if(istype(job, GetJob("Guwniak"))) // We don't want to give him assistant, that's boring!
			continue
...
/datum/subsystem/job/proc/DivideOccupations()
	...
	for(var/mob/new_player/player in unassigned)
		if(PopcapReached())
			RejectPlayer(player)
		else if(jobban_isbanned(player, "Guwniak"))
...
/datum/subsystem/job/proc/EquipRank(mob/living/H, rank, joined_late=0)
	...
	H << "<b>Twoja praca to [rank].</b>"
	H << "<b>Jako [rank] twoim szefem jest [job.supervisors]. W wyjatkowych sytuacjach moze sie to zmienic.</b>"
	H << "<b>By mowic na kanale radiowym twojego dzialu, uzywaj :h.</b>"
	if(job.req_admin_notify)
		H << "<b>Grasz klasa ktora jest wazna i potrzebna w wielu sytuacjach. Postaraj sie nie byc gownem.</b>"
	if(config.minimal_access_threshold)
		H << "<FONT color='blue'><B>Poniewaz ta stacja [config.jobs_have_minimal_access ? "ma wystarczajaco duzo personelu, tylko podstawowy dostep potrzebny do wykonania twojej pracy" : "ma niewystarczajaco duzo personelu, mozliwe ze dodatkowy dostep"] zostal dodany do twojego identyfikatora.</B></font>"
	return 1


/datum/subsystem/job/proc/setup_officer_positions()
	var/datum/job/J = SSjob.GetJob("Bagieta")
	if(!J)
