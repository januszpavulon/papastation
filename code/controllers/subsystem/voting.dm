...
/datum/subsystem/vote/proc/get_result()
	...
	if(!config.vote_no_default && choices.len)
		...
		if(non_voters > 0)
			if(mode == "restart")
				choices["Gramy dalej"] += non_voters
				if(choices["Gramy dalej"] >= greatest_votes)
					greatest_votes = choices["Gramy dalej"]
...
/datum/subsystem/vote/proc/announce_result()
	...
	if(winners.len > 0)
		...
		if(mode != "custom")
			if(winners.len > 1)
				text = "\n<b>Remis w glosowaniu pomiedzy:</b>"
				for(var/option in winners)
					text += "\n\t[option]"
			. = pick(winners)
			text += "\n<b>Wynik glosowania: [.]</b>"
		else
			text += "\n<b>Nie glosowalo:</b> [clients.len-voted.len]"
	else
		text += "<b>Nikt nie zaglosowal, glosowanie anulowane.</b>"
	log_vote(text)
	...
	if(restart)
		...
		for(var/client/C in admins)
			if(!C.is_afk() && check_rights_for(C, R_SERVER))
				...
				break
		if(!active_admins)
			world.Reboot("Glosowanie udane. Serwer restartuje sie.", "end_error", "restart vote")
		else
			world << "<span style='boldannounce'>Uwaga: glosowanie za restartem nie zrestartuje serwera automatycznie poniewaz znajduja sie na nim aktywni adminowie.</span>"
			message_admins("Glosowanie za restartem udalo sie, ale poniewaz na serwerze znajduja sie aktywni adminowie nie dokona restartu. Jesli chcesz, mozesz to zrobic recznie.")
...
/datum/subsystem/vote/proc/initiate_vote(vote_type, initiator_key)
	if(!mode)
		...
		switch(vote_type)
			if("restart")
				choices.Add("Restart","Gramy dalej")
			if("gamemode")
		...
		var/text = "Glosowanie o [capitalize(mode)] zostalo rozpoczete przez [initiator]."
		if(mode == "custom")
			text += "\n[question]"
		log_vote(text)
