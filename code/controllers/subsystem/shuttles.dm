...
/datum/subsystem/shuttle/proc/requestEvac(mob/user, call_reason)
	...
	if(world.time - round_start_time < config.shuttle_refuel_delay)
		user << "Statek ewakuacyjny jest w tej chwili niedostepny. Poczekaj [abs(round(((world.time - round_start_time) - config.shuttle_refuel_delay)/600))] minut."
		return

	switch(emergency.mode)
		if(SHUTTLE_RECALL)
			user << "Statek ewakuacyjny wlasnie wraca z powrotem do centrali. Nie mozesz go teraz wezwac."
			return
		if(SHUTTLE_CALL)
			user << "Statek ewakuacyjny juz zostal wezwany."
			return
		if(SHUTTLE_DOCKED)
			user << "Statek ewakuacyjny juz tu jest."
			return
		if(SHUTTLE_ESCAPE)
			user << "Statek ewakuacyjny juz odlecial."
			return
		if(SHUTTLE_STRANDED)
			user << "Zarzad odmawia wyslania statku ewakuacyjnego."
			return

	call_reason = trim(html_encode(call_reason))

	if(length(call_reason) < CALL_SHUTTLE_REASON_LENGTH)
		user << "Musisz podac przyczyne."
		return
...
/datum/subsystem/shuttle/proc/autoEvac()
	...
	if(callShuttle)
		if(emergency.mode < SHUTTLE_CALL)
			...
			message_admins("Wszystkie konsole komunikacyjne zostaly zniszczone a stacja nie posiada AI. Wezwano automatycznie statek ewakuacyjny.")
