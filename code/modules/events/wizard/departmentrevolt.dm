...
/datum/round_event/wizard/deprevolt/start()
	...
	switch(tidecolor)
		if("grey") //God help you
			jobs_to_revolt = list("Guwniak")
			nation = pick("Assa", "Mainte", "Tunnel", "Gris", "Grey", "Liath", "Grigio", "Ass", "Assi")
		if("white")
			jobs_to_revolt = list("Ordynator", "Lekarz", "Chemik", "Genetyk", "Wirusolog")
			nation = pick("Mede", "Healtha", "Recova", "Chemi", "Geneti", "Viro", "Psych")
		if("yellow")
			jobs_to_revolt = list("Glowny Inzynier", "Inzynier", "Technik Atmosferyczny")
			nation = pick("Atomo", "Engino", "Power", "Teleco")
		if("purple")
			jobs_to_revolt = list("Dyrektor ds. Naukowych","Naukowiec", "Robotyk")
			nation = pick("Sci", "Griffa", "Explosi", "Mecha", "Xeno")
		if("brown")
			jobs_to_revolt = list("Kwatermistrz", "Magazynier", "Gornik")
		...
		if("whatevercolorrepresentstheservicepeople") //the few, the proud, the technically aligned
			jobs_to_revolt = list("Barman", "Kucharz", "Botanik", "Klaun", "Mim", "Wozny", "Kaplan")
