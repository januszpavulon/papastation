...
/obj/machinery/computer/emergency_shuttle/emag_act(mob/user)
	if(!emagged && SSshuttle.emergency.mode == SHUTTLE_DOCKED)
		...
		minor_announce("Statek ewakuacyjny startuje za 10 sekund", "BLAD SYSTEMU:",null,1)
		SSshuttle.emergency.setTimer(100)
		emagged = 1
...
/obj/docking_port/mobile/emergency/request(obj/docking_port/stationary/S, coefficient=1, area/signalOrigin, reason, redAlert)
	...
	priority_announce("Statek ewakuacyjny zostal wezwany. [redAlert ? "Red Alert state confirmed: Wyslany zostal w tempie priorytetowym. " : "" ]Przyleci za [timeLeft(600)] minut.[reason][SSshuttle.emergencyLastCallLoc ? "\n\nMiejsce z ktorego zostal wezwany moze zostac sprawdzone w konsoli komunikacyjnej." : "" ]", null, 'sound/AI/shuttlecalled.ogg', "Uwaga!")
...
/obj/docking_port/mobile/emergency/cancel(area/signalOrigin)
	...
	if(prob(70))
		...
		SSshuttle.emergencyLastCallLoc = null
	priority_announce("Statek ewakuacyjny zostal odeslany.[SSshuttle.emergencyLastCallLoc ? " Miejsce z ktorego zostal odeslany moze zostac sprawdzone w konsoli komunikacyjnej." : "" ]", null, 'sound/AI/shuttlerecalled.ogg', "Uwaga!")
	...
	switch(mode)
		...
		if(SHUTTLE_CALL)
			if(time_left <= 0)
				...
				mode = SHUTTLE_DOCKED
				timer = world.time
				send2irc("Server", "Statek ewakuacyjny przylecial na stacje.")
				priority_announce("Statek ewakuacyjny zadokowal do stacji. Macie [timeLeft(600)] minut by dostac sie na jego poklad.", null, 'sound/AI/shuttledock.ogg', "Uwaga!")
		...
		if(SHUTTLE_DOCKED)
			...
			if(time_left <= 0 && !SSshuttle.emergencyNoEscape)
				...
				timer = world.time
				priority_announce("Statek ewakuacyjny opuscil stacje. Przylot do Centrali za okolo [timeLeft(600)] minut.", null, null, "Uwaga!")
		if(SHUTTLE_ESCAPE)
			if(time_left <= 0)
