...
/datum/job/warden
	title = "Naczelnik"
	flag = WARDEN
	department_head = list("Szef Ochrony")
	department_flag = ENGSEC
	faction = "Station"
	total_positions = 1
	spawn_positions = 1
	supervisors = "szef ochrony"
		...
		if(spawn_point)
			...
			while(safety < 25)
				...
				if(T && !H.Move(T))
					...
					break
	if(department)
		H << "<b>Zostales przydzielony do [department]!</b>"
	else
		H << "<b>Nie zostales przydzielony do zadnego departamentu.</b>"
