...
/mob/new_player/proc/new_player_panel()
	...
	var/output = "<center><p><a href='byond://?src=\ref[src];show_preferences=1'>Ustawienia Postaci</A></p>"

	if(!ticker || ticker.current_state <= GAME_STATE_PREGAME)
		if(ready)
			output += "<p>\[ <b>Gotowy</b> | <a href='byond://?src=\ref[src];ready=0'>Niegotowy</a> \]</p>"
		else
			output += "<p>\[ <a href='byond://?src=\ref[src];ready=1'>Gotowy</a> | <b>Niegotowy</b> \]</p>"

	else
		output += "<p><a href='byond://?src=\ref[src];manifest=1'>Lista Czlonkow Zalogi</A></p>"
		output += "<p><a href='byond://?src=\ref[src];late_join=1'>Dolacz do Gry!</A></p>"

	output += "<p><a href='byond://?src=\ref[src];observe=1'>Obserwuj</A></p>"
	...
	var/datum/browser/popup = new(src, "playersetup", "<div align='center'>Lobby</div>", 220, 265)
...
/mob/new_player/Stat()
	...
	if(statpanel("Lobby"))
		stat("Tryb gry:", (ticker.hide_mode) ? "Secret" : "[master_mode]")
		stat("Mapa:", MAP_NAME)

		if(ticker.current_state == GAME_STATE_PREGAME)
			stat("Czas do startu:", (ticker.timeLeft >= 0) ? "[round(ticker.timeLeft / 10)]s" : "ZATRZYMANE")

			stat("Gracze:", "[ticker.totalPlayers]")
			if(client.holder)
				stat("Gotowi Gracze:", "[ticker.totalPlayersReady]")
...
/mob/new_player/proc/LateChoices()
	...
	var/dat = "<div class='notice'>Dlugosc Rundy: [round(hours)]h [round(mins)]m</div>"

	switch(SSshuttle.emergency.mode)
		if(SHUTTLE_ESCAPE)
			dat += "<div class='notice red'>Statek ewakuacyjny odlecial ze stacji.</div><br>"
		if(SHUTTLE_CALL)
			if(!SSshuttle.canRecall())
				dat += "<div class='notice red'>Na stacji wlasnie maja miejsce procedury ewakuacyjne.</div><br>"
	...
	dat += "<div class='clearBoth'>Wybierz posade:</div><br>"
