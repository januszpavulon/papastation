/mob/living/carbon/monkey/emote(act,m_type=1,message = null)
	...
	switch(act) //Ooh ooh ah ah keep this alphabetical ooh ooh ah ah!
		if ("deathgasp","deathgasps")
			message = "<b>[src]</b> wydaje cichy dzwiek, po czym przestaje sie ruszac..."
			m_type = 1
		...
		if ("roar","roars")
			if (!muzzled)
				message = "<B>[src]</B> ryczy."
				m_type = 2

		if ("roll","rolls")
			if (!src.restrained())
				message = "<B>[src]</B> kreci sie."
				m_type = 1

		if ("scratch","scratches")
			if (!src.restrained())
				message = "<B>[src]</B> drapie sie."
				m_type = 1

		if ("screech","screeches")
			if (!muzzled)
				message = "<B>[src]</B> skrzeczy."
				m_type = 2
