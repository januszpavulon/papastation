/mob/living/carbon/human/emote(act,m_type=1,message = null)
	...
	switch(act) //Please keep this alphabetically ordered when adding or changing emotes.
		...
		if ("gasp","gasps")
			if (miming)
				message = "<B>[src]</B> wyglada jakby sie dusil"
			else
				..(act)

		if ("giggle","giggles")
			if (miming)
				message = "<B>[src]</B> chichocze cicho."
			else
				..(act)
		...
		if ("moan","moans")
			if(miming)
				...
				message = "<B>[src]</B> jeczy!"
				m_type = 2

		if ("mumble","mumbles")
			message = "<B>[src]</B> belkocze!"
			m_type = 2

		if ("pale")
			message = "<B>[src]</B> na chwile blednie."
			m_type = 1

		if ("raise")
			if (!src.restrained())
				message = "<B>[src]</B> podnosi reke."
			m_type = 1
		...
		if ("salute","salutes")
			if (!src.buckled)
				...
				if (!M)
					param = null
				if (param)
					message = "<B>[src]</B> salutuje do [param]."
				else
					message = "<B>[src]</b> salutuje."
			m_type = 1
		...
		if ("shiver","shivers")
			message = "<B>[src]</B> ma dreszcze."
			m_type = 1

		if ("shrug","shrugs")
			message = "<B>[src]</B> wzrusza ramionami."
			m_type = 1
		...
		if ("snore","snores")
			if (miming)
				message = "<B>[src]</B> spi glosno."
			else
				..(act)

		if ("whimper","whimpers")
			if (miming)
				message = "<B>[src]</B> wyglada na rannego."
			else
				..(act)
