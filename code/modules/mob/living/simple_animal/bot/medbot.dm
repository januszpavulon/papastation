...
/mob/living/simple_animal/bot/medbot/attackby(obj/item/weapon/W as obj, mob/user as mob, params)
	if(istype(W, /obj/item/weapon/reagent_containers/glass))
		if(locked)
			user << "<span class='warning'>Nie mozesz wlozyc pojemnika poniewaz panel jest zamkniety!</span>"
			return
		if(!isnull(reagent_glass))
			user << "<span class='warning'>W srodku juz jest jakis pojemnik!</span>"
			return
		if(!user.drop_item())
			return
		...
		W.loc = src
		reagent_glass = W
		user << "<span class='notice'>Wkladasz do srodka [W].</span>"
		show_controls(user)
		return
...
/mob/living/simple_animal/bot/medbot/process_scan(mob/living/carbon/human/H)
	...
	if(assess_patient(H))
		...
		if((last_newpatient_speak + 300) < world.time) //Don't spam these messages!
			var/message = pick("Hej, [H.name]! Poczekaj, chce ci pomoc.","Czekaj [H.name]! Pomoge ci!","[H.name], wygladasz na rannego!")
			speak(message)
...
/mob/living/simple_animal/bot/medbot/handle_automated_action()
	...
	if(!patient)
		if(!shut_up && prob(1))
			var/message = pick("Zesralem sie i jebie.","Ladny gabinet, szkoda zeby sie spalil.","Medbaze skonczylem z najnizsza krajowa!","Ehh, robomedanoni...")
			speak(message)
