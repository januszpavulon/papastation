...
/mob/living/carbon/human/interactive/proc/doSetup()
	...
	switch(myjob.title)
		...
		if("Barman")
			...
			functions += "bartend"
			restrictedJob = 1
		if("Inzynier","Glowny Inzynier","Technik Atmosferyczny")
			favoured_types = list(/obj/item/stack, /obj/item/weapon, /obj/item/clothing)
		if("Ordynator","Lekarz","Chemik","Wirusolog","Genetyk")
...
/mob/living/carbon/human/interactive/proc/job2area(target)
	...
	if(T.title == "Kapitan" || T.title == "Szef Personelu")
		return /area/bridge
	if(T.title == "Barman")
		return /area/crew_quarters/bar
	if(T.title == "Kucharz")
		return /area/crew_quarters/kitchen
	if(T.title == "Inzynier" || T.title == "Glowny Inzynier" || T.title == "Technik Atmosferyczny")
		return /area/engine
	if(T.title == "Ordynator" || T.title == "Lekarz" || T.title == "Chemik" || T.title == "Wirusolog" || T.title == "Genetyk")
		return /area/medical
	if(T.title == "Dyrektor ds. Naukowych" || T.title == "Naukowiec" || T.title == "Robotyk")
		return /area/toxins
	if(T.title == "Szef Ochrony" || T.title == "Naczelnik" || T.title == "Pracownik Ochron" || T.title == "Detektyw")
