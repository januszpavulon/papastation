...
/datum/preferences/proc/ShowChoices(mob/user)
	...
	dat += "<a href='?_src_=prefs;preference=tab;tab=0' [current_tab == 0 ? "class='linkOn'" : ""]>Ustawienia Postaci</a> "
	dat += "<a href='?_src_=prefs;preference=tab;tab=1' [current_tab == 1 ? "class='linkOn'" : ""]>Ustawienia Ogolne</a>"
	...
	switch(current_tab)
		if (0) // Character Settings#
			...
			dat += "<center><h2>Preferencje Zawodowe</h2>"
			dat += "<a href='?_src_=prefs;preference=job;task=menu'>Ustaw Preferencje Zawodowe</a><br></center>"
			dat += "<h2>Tozsamosc</h2>"
			dat += "<table width='100%'><tr><td width='75%' valign='top'>"
			if(appearance_isbanned(user))
				dat += "<b>You are banned from using custom names and appearances. You can continue to adjust your characters, but you will be randomised once you join the game.</b><br>"
			dat += "<a href='?_src_=prefs;preference=name;task=random'>Losowe Imie i Nazwisko</A> "
			dat += "<a href='?_src_=prefs;preference=name'>Imie i Nazwisko zawsze losowe: [be_random_name ? "Tak" : "Nie"]</a><BR>"

			dat += "<b>Imie i Nazwisko:</b> "
			dat += "<a href='?_src_=prefs;preference=name;task=input'>[real_name]</a><BR>"

			dat += "<b>Plec:</b> <a href='?_src_=prefs;preference=gender'>[gender == MALE ? "Mezczyzna" : "Kobieta"]</a><BR>"
			dat += "<b>Wiek:</b> <a href='?_src_=prefs;preference=age;task=input'>[age]</a><BR>"

			dat += "<b>Imiona Specjalne:</b><BR>"
			dat += "<a href ='?_src_=prefs;preference=clown_name;task=input'><b>Klaun:</b> [custom_names["clown"]]</a> "
			dat += "<a href ='?_src_=prefs;preference=mime_name;task=input'><b>Mim:</b>[custom_names["mime"]]</a><BR>"
			...
			dat += "<a href ='?_src_=prefs;preference=religion_name;task=input'><b>Religia jako kaplan:</b> [custom_names["religion"]] </a>"
			dat += "<a href ='?_src_=prefs;preference=deity_name;task=input'><b>Bostwo jako kaplan:</b> [custom_names["deity"]]</a><BR></td>"
			...
			dat += "<h2>Wyglad</h2>"
			dat += "<a href='?_src_=prefs;preference=all;task=random'>Losowy Wyglad</A> "
			dat += "<a href='?_src_=prefs;preference=all'>Zawsze Losowy Wyglad: [be_random_body ? "Tak" : "Nie"]</A><br>"

			dat += "<table width='100%'><tr><td width='24%' valign='top'>"

			if(config.mutant_races)
				dat += "<b>Gatunek:</b><BR><a href='?_src_=prefs;preference=species;task=input'>[pref_species.name]</a><BR>"
			else
				dat += "<b>Gatunek:</b> Czlowiek<BR>"

			dat += "<b>Grupa Krwi:</b> [blood_type]<BR>"
			dat += "<b>Bielizna:</b><BR><a href ='?_src_=prefs;preference=underwear;task=input'>[underwear]</a><BR>"
			dat += "<b>Podkoszulek:</b><BR><a href ='?_src_=prefs;preference=undershirt;task=input'>[undershirt]</a><BR>"
			dat += "<b>Skarpety:</b><BR><a href ='?_src_=prefs;preference=socks;task=input'>[socks]</a><BR>"
			dat += "<b>Plecak:</b><BR><a href ='?_src_=prefs;preference=bag;task=input'>[backbag]</a><BR></td>"
			...
			if(HAIR in pref_species.specflags)
				...
				dat += "<span style='border:1px solid #161616; background-color: #[hair_color];'>&nbsp;&nbsp;&nbsp;</span> <a href='?_src_=prefs;preference=hair;task=input'>Zmien</a><BR>"
				...
				dat += "<span style='border: 1px solid #161616; background-color: #[facial_hair_color];'>&nbsp;&nbsp;&nbsp;</span> <a href='?_src_=prefs;preference=facial;task=input'>Zmien</a><BR>"
			...
			if(EYECOLOR in pref_species.specflags)
				...
				dat += "<h3>Kolor Oczu</h3>"

				dat += "<span style='border: 1px solid #161616; background-color: #[eye_color];'>&nbsp;&nbsp;&nbsp;</span> <a href='?_src_=prefs;preference=eyes;task=input'>Zmien</a><BR>"
			...
			if(config.mutant_races) //We don't allow mutant bodyparts for humans either unless this is true.
				...
				if((MUTCOLORS in pref_species.specflags) || (MUTCOLORS_PARTSONLY in pref_species.specflags))
					...
					dat += "<span style='border: 1px solid #161616; background-color: #[features["mcolor"]];'>&nbsp;&nbsp;&nbsp;</span> <a href='?_src_=prefs;preference=mutant_color;task=input'>Zmien</a><BR>"
		...
		if (1) // Game Preferences
			...
			dat += "<h2>Ustawienia Ogolne</h2>"
			dat += "<b>Styl UI:</b> <a href='?_src_=prefs;preference=ui'>[UI_style]</a><br>"
			dat += "<b>Skroty Klawiszowe:</b> <a href='?_src_=prefs;preference=hotkeys'>[(hotkeys) ? "Hotkeye" : "Bez Hotkeyow"]</a><br>"
			dat += "<b>Styl tgui:</b> <a href='?_src_=prefs;preference=tgui_fancy'>[(tgui_fancy) ? "Fancy" : "No Frills"]</a><br>"
			dat += "<b>Monitory tgui:</b> <a href='?_src_=prefs;preference=tgui_lock'>[(tgui_lock) ? "Primary" : "All"]</a><br>"
			dat += "<b>Graj muzyke puszczana przez adminow:</b> <a href='?_src_=prefs;preference=hear_midis'>[(toggles & SOUND_MIDI) ? "Tak" : "Nie"]</a><br>"
			dat += "<b>Graj muzyke w lobby:</b> <a href='?_src_=prefs;preference=lobby_music'>[(toggles & SOUND_LOBBY) ? "Tak" : "Nie"]</a><br>"
			dat += "<b>Uszy ducha:</b> <a href='?_src_=prefs;preference=ghost_ears'>[(chat_toggles & CHAT_GHOSTEARS) ? "Wszystkie kwestie" : "Tylko najblizsze kwestie"]</a><br>"
			dat += "<b>Wzrok ducha:</b> <a href='?_src_=prefs;preference=ghost_sight'>[(chat_toggles & CHAT_GHOSTSIGHT) ? "Wszystkie ruchy" : "Tylko najblizsze ruchy"]</a><br>"
			dat += "<b>Szept ducha:</b> <a href='?_src_=prefs;preference=ghost_whispers'>[(chat_toggles & CHAT_GHOSTWHISPER) ? "Wszystkie szepty" : "Tylko najblizsze szepty"]</a><br>"
			dat += "<b>Radio ducha:</b> <a href='?_src=prefs;preference=ghost_radio'>[(chat_toggles & CHAT_GHOSTRADIO) ? "Tak" : "Nie"]</a><br>"
			dat += "<b>PDA ducha:</b> <a href='?_src=prefs;preference=ghost_pda'>[(chat_toggles & CHAT_GHOSTPDA) ? "Wszystkie wiadomosci" : "Tylko najblizsze wiadomosci"]</a><br>"
			dat += "<b>Pull requests:</b> <a href='?_src_=prefs;preference=pull_requests'>[(chat_toggles & CHAT_PULLR) ? "Yes" : "No"]</a><br>"
			dat += "<b>Antagonista podczas trwania rundy:</b> <a href='?_src_=prefs;preference=allow_midround_antag'>[(toggles & MIDROUND_ANTAG) ? "Tak" : "Nie"]</a><br>"
			if(config.allow_Metadata)
				dat += "<b>Notatki OOC:</b> <a href='?_src_=prefs;preference=metadata;task=input'>Edit </a><br>"

			if(user.client)
				if(user.client.holder)
					dat += "<b>Dzwiek AHelp:</b> <a href='?_src_=prefs;preference=hear_adminhelps'>[(toggles & SOUND_ADMINHELP)?"On":"Off"]</a><br>"
					dat += "<b>Atencja przy logowaniu:</b> <a href='?_src_=prefs;preference=announce_login'>[(toggles & ANNOUNCE_LOGIN)?"On":"Off"]</a><br>"

				if(unlock_content || check_rights_for(user.client, R_ADMIN))
					dat += "<b>Kolor OOC:</b> <span style='border: 1px solid #161616; background-color: [ooccolor ? ooccolor : normal_ooc_colour];'>&nbsp;&nbsp;&nbsp;</span> <a href='?_src_=prefs;preference=ooccolor;task=input'>Change</a><br>"

				if(unlock_content)
					dat += "<b>Atencja posiadania czlonkowstwa BYOND:</b> <a href='?_src_=prefs;preference=publicity'>[(toggles & MEMBER_PUBLIC) ? "Public" : "Hidden"]</a><br>"
					dat += "<b>Wyglad ducha:</b> <a href='?_src_=prefs;task=input;preference=ghostform'>[ghost_form]</a><br>"
					dat += "<B>Styl orbitowania ducha: </B> <a href='?_src_=prefs;task=input;preference=ghostorbit'>[ghost_orbit]</a><br>"

			var/button_name = "Jesli to widzisz to cos sie zjebalo."
			switch(ghost_accs)
				if(GHOST_ACCS_FULL)
			...
			dat += "<b>Akcesoria ducha:</b> <a href='?_src_=prefs;task=input;preference=ghostaccs'>[button_name]</a><br>"
			...
			dat += "<b>Wyglad innych duchow:</b> <a href='?_src_=prefs;task=input;preference=ghostothers'>[button_name]</a><br>"
...
/datum/preferences/proc/SetChoices(mob/user, limit = 17, list/splitJobs = list("Chief Engineer"), widthPerColumn = 295, height = 620)
	...
	var/HTML = "<center>"
	HTML += "<b>Ustaw preferencje zawodowe</b><br>"
	HTML += "<div align='center'>Lewym przyciskiem podnosisz preferencje uzyskania danej posady, prawym przyciskiem zmniejszasz.<br></div>"
	HTML += "<center><a href='?_src_=prefs;preference=job;task=close'>Gotowe</a></center><br>" // Easier to press up here.
	...
	for(var/datum/job/job in SSjob.occupations)
		...
		if(!job.player_old_enough(user.client))
			...
			continue
		if((job_civilian_low & ASSISTANT) && (rank != "Guwniak") && !jobban_isbanned(user, "Guwniak"))
		...
		if(GetJobDepartment(job, 1) & job.flag)
			prefLevelLabel = "Wysokie"
			prefLevelColor = "slateblue"
			prefUpperLevel = 4
			prefLowerLevel = 2
		else if(GetJobDepartment(job, 2) & job.flag)
			prefLevelLabel = "Srednie"
			prefLevelColor = "green"
			prefUpperLevel = 1
			prefLowerLevel = 3
		else if(GetJobDepartment(job, 3) & job.flag)
			prefLevelLabel = "Niskie"
			prefLevelColor = "orange"
			prefUpperLevel = 2
			prefLowerLevel = 4
		else
			prefLevelLabel = "NIGDY"
			prefLevelColor = "red"
			prefUpperLevel = 3
			prefLowerLevel = 1
		...
		if(rank == "Guwniak")//Assistant is special
	...
	HTML += "<center><br><a href='?_src_=prefs;preference=job;task=random'>[userandomjob ? "Gdy preferencje sa niedostepne dostaniesz losowa posade" : "Gdy preferencje sa niedostepne zostaniesz Guwniakiem"]</a></center>"
	HTML += "<center><a href='?_src_=prefs;preference=job;task=reset'>Resetuj Ustawienia</a></center>"

	user << browse(null, "window=preferences")
	//user << browse(HTML, "window=mob_occupation;size=[width]x[height]")
	var/datum/browser/popup = new(user, "mob_occupation", "<div align='center'>Preferencje Zawodowe</div>", width, height)
	...
	if(href_list["preference"] == "job")
		switch(href_list["task"])
			...
			if("reset")
				...
				SetChoices(user)
			if("random")
				if(jobban_isbanned(user, "Guwniak"))
					userandomjob = 1
				else
